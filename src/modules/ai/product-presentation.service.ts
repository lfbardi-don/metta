...
} from './templates/product-presentation.templates';
...
export class ProductPresentationService {
  ...
  detectQueryContext(
    ...
    const isFollowUp =
      ...
      mentionedProducts.some((p) =>
        this.wasRecentlyMentioned(p.productId, conversationState),
      );
  ...
  determinePresentationMode(
    ...
    if (type === 'RE_SHOW') {
      this.logger.log(
        'Presentation mode: FULL_CARD (explicit re-show request)',
      );
      return 'FULL_CARD';
    }
    ...
    if (type === 'SIZE_QUERY') {
      if (mentionedProducts.length > 0) {
        ...
        this.logger.log(
          'Presentation mode: SIZE_ONLY (size query for known product)',
        );
        return 'SIZE_ONLY';
      } else {
        // Product never mentioned → FULL_CARD (first time showing it)
        this.logger.log(
          'Presentation mode: FULL_CARD (size query for new product)',
        );
        return 'FULL_CARD';
      }
    }
    ...
    if (type === 'COMPARISON') {
      if (isFollowUp) {
        this.logger.log(
          'Presentation mode: COMPACT (comparison of recent products)',
        );
        return 'COMPACT';
      } else {
        this.logger.log(
          'Presentation mode: FULL_CARD (comparison of non-recent products)',
        );
        return 'FULL_CARD';
      }
    }
    ...
    if (type === 'ATTRIBUTE_QUERY') {
      if (isFollowUp) {
        this.logger.log(
          'Presentation mode: TEXT_ONLY (attribute query for recent product)',
        );
        return 'TEXT_ONLY';
      } else {
        this.logger.log(
          'Presentation mode: FULL_CARD (attribute query for non-recent product)',
        );
        return 'FULL_CARD';
      }
    }
  ...
  generatePresentationInstructions(
    ...
    if (mentionedProducts.length > 0 && mode !== 'FULL_CARD') {
      const productInfo = mentionedProducts
        .map(
          (p) =>
  ...
  private wasRecentlyMentioned(
    ...
    const product = conversationState.state.products.find(
    ...
    this.logger.debug(
      `Product ${productId} mentioned ${diffMinutes.toFixed(1)} minutes ago - Recent: ${isRecent}`,
    );
  ...
  private extractMentionedProducts(
    ...
    const positionalMatch = userMessage.match(
